$(function() {
	...
	var remarkEditor = editormd("remark", {
		...
		toolbarIcons : function() {
			...
			return ["undo","redo","|","bold","del","italic","quote","uppercase","lowercase","|","h1","h2","h3","h4","h5","h6","|","list-ul","list-ol","hr","|","code-block","table","|","watch","preview","fullscreen","|","help","info"]
		},
        placeholder      : "请输入备注"
	});
	
	var respRemarkEditor = editormd("respRemark", {
		width   : "100%",
		height  : 350,
		syncScrolling : "single",
		path    : base_url + "/static/plugins/editor.md-1.5.0/lib/",
	...
	var reqRemarkEditor = editormd("reqRemark", {
		width   : "100%",
		height  : 350,
		syncScrolling : "single",
		path    : base_url + "/static/plugins/editor.md-1.5.0/lib/",
	 ...
	 * 依赖资源，新增一行
	 */
	$('#needResource_add').click(function () {
		var html = $('#needResource_example').html();
		$('#needResource_parent').append(html);

		$("#needResource_parent .select2_tag_new").each(function () {
			var $select2 = $(this);
			$($select2).removeClass('select2_tag_new');
			$($select2).addClass('select2_tag');
			$($select2).select2();
	 ...
	 * 依赖资源，删除一行
	 */
	$('#needResource_parent').on('click', '.delete',function () {
		$(this).parents('.needResource_item').remove();
	});
	...
	var addModalValidate = $("#ducomentForm").validate({
		...
		messages : {
			...
			wikiparenturl : {
				minlength: "长度不可少于60,参考:https://de-wiki.dataenlighten.com/pages/viewpage.action?pageId=00000000",
				maxlength: "长度不可多余80,参考url:https://de-wiki.dataenlighten.com/pages/viewpage.action?pageId=00000000"
			},
			wikiurl : {
				minlength: "长度不可少于60,参考:https://de-wiki.dataenlighten.com/pages/viewpage.action?pageId=00000000",
				maxlength: "长度不可多余80,参考:https://de-wiki.dataenlighten.com/pages/viewpage.action?pageId=00000000"
			}
		},
		...
		submitHandler : function(form) {
			...
			var respRemark = respRemarkEditor.getMarkdown();
			var reqRemark = reqRemarkEditor.getMarkdown();
			...
			if($('#wikiparenturl')!=null && $('#wikiparenturl').val()!=''){
			   wikiParentId = getUrlParamByUrl($('#wikiparenturl').val(),'pageId');
			}
			...
			// needResource
			var needResourceList = new Array();
			if ($('#needResource_parent').find('.needResource_item').length > 0) {
				$('#needResource_parent').find('.needResource_item').each(function () {
					var notNull = $(this).find('.notNull').val();
					var type = $(this).find('.type').val();
					var name = $(this).find('.name').val();
					var desc = $(this).find('.desc').val();
					if (name) {
						needResourceList.push({
							'notNull':notNull,
							'type':type,
							'name':name,
							'desc':desc
			...
			var needResources = JSON.stringify(needResourceList);
	...
	// JSON 格式化并校验
    $('#requestExample_2json').click(function () {
        try {
            var jsonStr = $('#requestExample').val();
