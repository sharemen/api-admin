$(function() {
	 ...
	 * remark view
	 */
	respRemarkView = editormd.markdownToHTML("respRemark", {
		htmlDecode      : "style,script,iframe",  // you can filter tags decode
		emoji           : false,
		taskList        : false,
		tex             : false,  // 默认不解析
	 ...
	 * 设置默认，Mock数据
	 */
	$('.defaultMock').click(function () {
		var id = $(this).attr('_id');

        layer.confirm( "确认将该Mock数据设为默认?" , {
            icon: 3,
            title: '系统提示' ,
            btn: [ '确定', '取消' ]
        }, function(index){
