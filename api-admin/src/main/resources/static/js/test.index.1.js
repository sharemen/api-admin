$(function() {
	...
	$('#projectBaseUrlUpdate').change(function () {
		...
		if($(this).find("option:selected").text() == '开发环境' || $(this).find("option:selected").text() == '测试环境'){
			$('#withTestToken').prop("checked",true);  
			$('#isRequestBody').prop("checked",true);  
		}else{
			
			if($(this).find("option:selected").text() == '线上环境' && $("#enableProdRun").val() == '1'){
					$('#withTestToken').prop("checked",true);  
					$('#isRequestBody').prop("checked",true); 
					
				}else{
					$('#withTestToken').prop("checked",false);  
					$('#isRequestBody').prop("checked",false); 
				}
		}
	...
	$('#run').click(function () {
		...
		if ($('#isRequestBody').prop("checked")){
			isRequestBody = $('#isRequestBody').val();
		}
	
		var withTestToken = '';
		if ($('#withTestToken').prop("checked")){
			withTestToken = $('#withTestToken').val();
		}
		...
		if ($('#queryParams_parent').find('.queryParams_item').length > 0) {
			$('#queryParams_parent').find('.queryParams_item').each(function () {
				var key = $(this).find('.key').val();
				var value = $(this).find('.value').val();
				var valuetype = $(this).find('.valuetype').val();
				//转换参数值的对象类型
				if("boolean" == valuetype.toLowerCase()){
					value =  Boolean(value)
				}else
				if("int" == valuetype.toLowerCase()){
					value =  Number(value)
				}else
				if("long" == valuetype.toLowerCase()){
						value =  Number(value)
				}else
				if("float" == valuetype.toLowerCase()){
						value =  Number(value)
				}else
				if("double" == valuetype.toLowerCase()){
						value =  Number(value)
				}	
				if (key) {
					queryParamList.push({
