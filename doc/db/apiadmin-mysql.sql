CREATE database if NOT EXISTS `apiadmin` default character set utf8 collate utf8_general_ci;
use `apiadmin`;


CREATE TABLE `api_biz` (
...
CREATE TABLE `api_datatype_fileds` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
...
CREATE TABLE `api_document` (
  ...
  `need_resources` varchar(2000) COMMENT '依赖资源JSON',
  `wiki_id`  int(11) COMMENT 'api对应的wiki文档id',
  `response_remark` text COMMENT '响应备注：描述json格式的细节等',
  `creat_user` varchar(255) COMMENT 'api创建用户',
  `update_user` varchar(255) COMMENT 'api最后修改用户',
  `request_remark` text COMMENT '参数备注：描述json格式的细节等',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

CREATE TABLE `api_history` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `document_id` int(11) NOT NULL COMMENT 'ApiID',
  `oper_user` varchar(255) NOT NULL COMMENT '操作用户',
...
CREATE TABLE `api_group` (
  ...
  `order_index` int(11) NOT NULL COMMENT '分组排序',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `api_mock` (
  ...
  `req_uri` varchar(2000)  COMMENT 'mock对应的api uri',
  `isdefault` int(2)  COMMENT '是否为默认：0否 ,1是',
...
CREATE TABLE `api_project` (
  ...
  `base_url_dev` varchar(200)  DEFAULT NULL COMMENT '根地址：开发环境',
...
('1', 'admin', 'e10adc3949ba59abbe56e057f20f883e', '1', ''), -- 默认密码 123456
('2', 'user', 'e10adc3949ba59abbe56e057f20f883e', '0', '1'), -- 默认密码 123456
('3', 'guest', 'e10adc3949ba59abbe56e057f20f883e','0', '1'); -- 默认密码 123456

INSERT INTO `api_biz` VALUES ('1', '默认业务线', '1');

INSERT INTO `api_datatype` VALUES
